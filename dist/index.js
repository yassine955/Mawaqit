...
async function firstTimeStart() {
    try {
        ...
        await sound_play_1.default.play(path_1.default.join(__dirname, "../src/audio/beep.mp3"), 1);
    }
    catch (error) {
...
(async () => {
    ...
    async function scrapePrayerTimes() {
        ...
        prayerTimes = await page.evaluate(() => {
            ...
            const times = Array.from(timeElements).map((element) => element.textContent);
            return times;
        });
    ...
    async function checkPrayerTimes() {
        ...
        if (prayerTimes.includes(currentTime)) {
            ...
            try {
                await sound_play_1.default.play(path_1.default.join(__dirname, "../src/audio/azan5.mp3"), 1);
                console.log(`Audio is playing now!`);
            }
            catch (error) {
                console.error("Error playing audio:", error);
    ...
    node_cron_1.default.schedule("* * * * *", runTask);
})();
